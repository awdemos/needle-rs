...
geo, ts = read_export("/var/home/a/code/needle-rs/models/tiny.cact")
...
z = xin
z = kron_apply(lw["d1"] * z, lw["w1a"], lw["w1b"])[p1.astype(int)]
pre2 = lw["d2"] * cond * z + lw["b2"]
z = kron_apply(pre2 / (1 + np.exp(-pre2)), lw["w2a"], lw["w2b"])[p2.astype(int)]
z = kron_apply(lw["d3"] * z, lw["w3a"], lw["w3b"])
hada = (lw["d4"] * z)[:d]
