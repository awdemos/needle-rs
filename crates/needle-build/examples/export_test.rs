...
fn main() {
    ...
    println!("loaded checkpoint in {:?}: layers={} engram sites={:?} global={:?}",
        t0.elapsed(), ckpt.config.num_layers, ckpt.config.engram_layers, ckpt.config.global_layers);

    let ar = needle_format::read_archive(std::path::Path::new("models/needle3.cact")).unwrap();
    let blob = ar.tokenizer_blob().unwrap().to_vec();

    ckpt.ladder_slice(8).unwrap();
    println!("sliced to {} layers; engram sites {:?}", ckpt.config.num_layers, ckpt.config.engram_layers);

    let t1 = std::time::Instant::now();
    ckpt.write_cact(std::path::Path::new("models/needle3-8l.cact"), Some(&blob), 4, 128).unwrap();
    println!("exported in {:?} -> models/needle3-8l.cact ({} bytes)",
        t1.elapsed(),
        std::fs::metadata("models/needle3-8l.cact").unwrap().len());

    let ar2 = needle_format::read_archive(std::path::Path::new("models/needle3-8l.cact")).unwrap();
