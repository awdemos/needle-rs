...
pub fn config_from_json(v: &serde_json::Value) -> Config {
    ...
    let gmask: u64 = global.iter().filter(|&&g| g < 64).map(|&g| 1u64 << g).sum();
    Config {
...
impl Checkpoint {
    ...
    pub fn merge_lora(&mut self, adapter: &Safetensors, scale: f32) -> Result<usize, String> {
        ...
        for name in adapter.tensors.keys() {
            ...
            let target = path.strip_prefix("lora/").unwrap();
            if which == "B" {
                continue; // handled with the A side
            }
            let a = adapter.get(&format!("{path}/A")).ok_or("missing A")?.to_vec();
            let b = adapter.get(&format!("{path}/B")).ok_or("missing B")?.to_vec();
            let shape = self.tensors.shape(target).unwrap().to_vec();
            let w = self
                .tensors
                .tensors
                .get_mut(target)
            ...
            let rank = adapter.shape(&format!("{path}/A")).unwrap()[adapter.shape(&format!("{path}/A")).unwrap().len() - 1];
            for l in 0..lead {
    ...
    pub fn ladder_slice(&mut self, depth: usize) -> Result<(), String> {
        ...
        for name in self.tensors.tensors.keys().cloned().collect::<Vec<_>>() {
            if name.starts_with("stack/layers/block/") || name.starts_with("stack/mhc_") {
                ...
                if shape.len() >= 1 && shape[0] == num_layers {
        ...
        for head in ["embedding_head", "confidence_head", "router_head"] {
            ...
            for sub in ["probes", "gain"] {
                ...
                let shape = self.tensors.shape(&key).unwrap().to_vec();
                let data = self.tensors.tensors.get(&key).unwrap();
            ...
            let shape = self.tensors.shape(&rb_key).unwrap().to_vec();
            let data = self.tensors.tensors.get(&rb_key).unwrap();
            let q = shape[0];
            let l1 = shape[1];
            let k = shape[2];
...
pub fn cq_pack(w: &[f32], out: usize, inp: usize, bits: u32, group: usize, cb: &[f32]) -> CqMat {
    ...
    for row in 0..out {
        for g in 0..groups {
            for k in 0..group {
                let c = g * group + k;
                tmp[k] = if c < inp { w[row * inp + c] } else { 0.0 };
            }
            // rot = tmp @ H (symmetric)
...
impl Checkpoint {
    ...
    pub fn write_cact(
        ...
        for l in 0..c.num_layers {
            ...
            let li = |suffix: &str| -> Vec<f32> {
                let full = at(suffix);
                let shape = t.shape(&full).unwrap().to_vec();
                let per: usize = shape.iter().product::<usize>() / shape[0];
                let data = t.get(&full).unwrap();
        ...
        for name in ["mhc_a_pre", "mhc_a_post", "mhc_a_res", "mhc_b_pre", "mhc_b_post", "mhc_b_res"] {
            let data = t.get(&format!("stack/{name}")).ok_or(name)?;
            ts.push(fp16_tensor(t.shape(&format!("stack/{name}")).unwrap(), data));
        }
        let n = c.mhc_lanes;
        let nc = n * d;
        for name in ["mhc_phi_pre", "mhc_phi_post", "mhc_phi_res"] {
            let data = t.get(&format!("stack/{name}")).ok_or(name)?; // (L, nC, X)
            let x = data.len() / (c.num_layers * nc);
        ...
        for s in 0..c.engram_layers.len() {
            let tables = t.get(&format!("engrams_{s}/embedding")).ok_or("engram tables")?;
            ts.push(cq_tensor(tables, num_tables * c.engram_slots, sub, bits, group));
            for proj in ["key_proj", "value_proj"] {
                let w = t.get(&format!("engrams_{s}/{proj}/kernel")).ok_or("engram proj")?;
                let wt = transpose(w, num_tables * sub, d);
                ts.push(cq_tensor(&wt, d, num_tables * sub, bits, group));
            }
            let taps = t.get(&format!("engrams_{s}/taps")).ok_or("engram taps")?;
            ts.push(fp16_tensor(&[4, d], taps));
        }

        ts.push(fp16_tensor(&[d], t.get("stack/final_norm/scale").ok_or("final_norm")?));
        ...
        if !head_codes.is_empty() {
            ...
            for (name, _) in [("embedding_head", 1.0), ("confidence_head", 2.0), ("router_head", 3.0)] {
                ...
                let probes = t.get(&format!("{name}/probes")).unwrap(); // (rows, k, d)
                let k = probes.len() / (rows * d);
                ts.push(cq_tensor(probes, rows * k, d, bits, group));
                let gain = t.get(&format!("{name}/gain")).unwrap();
                ts.push(fp16_tensor(&[rows, k], gain));
                let query = t.get(&format!("{name}/query")).unwrap();
                let q = query.len() / d;
                ts.push(cq_tensor(query, q, d, bits, group));
                let rb = t.get(&format!("{name}/row_bias")).unwrap();
                ts.push(fp16_tensor(&[q, rows, k], rb));
                let proj = t.get(&format!("{name}/proj/kernel")).unwrap();
                let out_dim = proj.len() / (q * d);
                ts.push(cq_tensor(proj, out_dim, q * d, bits, group));
                let bias = t.get(&format!("{name}/proj/bias")).map(|b| b.to_vec()).unwrap_or(vec![0.0; out_dim]);
        ...
        let mut u32le = |v: u32, h: &mut Vec<u8>| h.extend_from_slice(&v.to_le_bytes());
